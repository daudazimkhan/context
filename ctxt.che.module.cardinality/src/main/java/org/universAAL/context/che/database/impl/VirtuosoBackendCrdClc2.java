...
import org.universAAL.context.rdf4j.sail.Collection2Store;

/**
 * Extension of
 * {@link org.universAAL.context.che.database.impl.VirtuosoBackendCrd} that uses a
 * modified Virtuoso Store that checks cardinality of stored statements,
 ...
 *
 */
public class VirtuosoBackendCrdClc2 extends VirtuosoBackend {
	/**
	 * Logger.
	 */
	private static Log log = Hub.getLog(VirtuosoBackendCrdClc2.class);
		...
		if (dataPath != null) {
			...
			log.error("connect",
					"No location specified for the store. Add and " + "specify the configuration parameter "
							+ "STORE.LOCATION to the configuration" + " file of the CHE pointing to a valid"
							+ " folder path.");
		}
	}
